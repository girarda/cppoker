...
{
    initPreFlopPair();
    pokerGame::Choice choice = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, A_BET, BIG_BLIND).choice;
...
{
    initPreFlopPair();
    float newBet = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, A_BET, BIG_BLIND).bet;
...
{
    initPostFlopPair();
    pokerGame::Choice choice = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, ZERO_BET, BIG_BLIND).choice;
    ASSERT_EQ(pokerGame::CHECK, choice);
}
...
{
    initPostFlopPair();
    float newBet = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, ZERO_BET, BIG_BLIND).bet;
    ASSERT_EQ(0, newBet);
}
