...
protected:
    ...
    static const float NUMBER_OF_RAISES;
...
const float SimpleBettingBluffStrategyTest::NUMBER_OF_RAISES(0);

TEST_F(SimpleBettingBluffStrategyTest, whenPreFlopIfHandValueIsAPairMakeDecisionReturnsCall)
{
    initPreFlopPair();
    pokerGame::Choice choice = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, A_BET, BIG_BLIND, NUMBER_OF_RAISES).choice;
...
{
    initPreFlopPair();
    float newBet = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, A_BET, BIG_BLIND, NUMBER_OF_RAISES).bet;
...
{
    initPostFlopPair();
    pokerGame::Choice choice = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, ZERO_BET, BIG_BLIND, NUMBER_OF_RAISES).choice;
    ASSERT_EQ(pokerGame::CHECK, choice);
}
...
{
    initPostFlopPair();
    float newBet = simpleBettingBluffStrategy->makeDecision(*hole, *sharedCards, ZERO_BET, BIG_BLIND, NUMBER_OF_RAISES).bet;
    ASSERT_EQ(0, newBet);
}
