...
TEST_F(PlayerTest, makingDecisionReturnsCheckWhenPlayerChecks)
{
    ...
    EXPECT_CALL(*aPlayerController, makeDecision(_, *sharedCards, 0, BIG_BLIND, NUMBER_OF_RAISES)).Times(1).WillOnce(Return(aDecision)); // TODO: _ should be player->hole

    pokerGame::Decision returnedDecision = aPlayer->makeDecision(0, BIG_BLIND, *sharedCards, NUMBER_OF_RAISES);
...
TEST_F(PlayerTest, makingDecisionAsksUserASecondTimesIfItDoesNotHaveEnoughMoneyForHisDecision)
{
    ...
    EXPECT_CALL(*aPlayerController, makeDecision(_, *sharedCards, 0, BIG_BLIND, NUMBER_OF_RAISES)).Times(2).WillOnce(Return(callDecision)).WillOnce(Return(checkDecision)); // TODO: _ should be player->hole
...
TEST_F(PlayerTest, makingDecisionReturnsCallWhenPlayerCallsAndHasEnoughMoney)
{
    ...
    EXPECT_CALL(*aPlayerController, makeDecision(_, *sharedCards, 0, BIG_BLIND, NUMBER_OF_RAISES)).Times(1).WillOnce(Return(callDecision)); // TODO: _ should be player->hole
    pokerGame::Decision returnedDecision = aPlayer->makeDecision(0, BIG_BLIND, *sharedCards, NUMBER_OF_RAISES);
...
TEST_F(PlayerTest, playerFoldsWhenDecisionIsToFold)
{
    ...
    EXPECT_CALL(*aPlayerController, makeDecision(_, *sharedCards, 0, BIG_BLIND, NUMBER_OF_RAISES)).Times(1).WillOnce(Return(foldDecision)); // TODO: _ should be player->hole
