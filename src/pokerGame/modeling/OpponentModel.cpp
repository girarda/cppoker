...
OpponentModel::OpponentModel(const modeling::ActionContext &lastActionContext, const std::vector<modeling::HandContext> &handContexts) : numberOfOccurrences(0), handStrengthAverage(0), handStrengthDeviation(0) {
    for (std::vector<modeling::HandContext>::const_iterator it = handContexts.begin(); it != handContexts.end(); it++) {
